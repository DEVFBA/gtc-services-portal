...
function ModalAddCustomUoM({modalAddRecord, setModalAddRecord, updateAddDataCustomUoMs, ip, setCustomUoME, toggleModalAddRecord, setMensajeAdd}) {
        // update form
            ...
            .then((data) => {
                if (data.errors) {
                    ...
                    if(data[0].Code_Type === "Warning")
                    {
                        ...
                        updateAddDataCustomUoMs();
                        //Para elegir en el modal principal la clave producto elegida
                        getCustomUoM(id);
                        //Cerramos el modal
                        handleModalClick()
                        setMensajeAdd("Unidad de Medida SAT creada con éxito.")
                        toggleModalAddRecord()
                    }
                }
            });
    ...
    function getCustomUoM(id){

        var url = new URL(`${process.env.REACT_APP_API_URI}cat-catalogs/get-custom-uom/${id}`);
        fetch(url, {
          method: "GET",
          headers: {
              "access-token": token,
        ...
        .then(function(data) {
            setCustomUoME({
                value: data[0].Id_Catalog, label: data[0].Id_Catalog + " - " + data[0].Short_Desc
            })
        })
        .catch(function(err) {
...
export default ModalAddCustomUoM;